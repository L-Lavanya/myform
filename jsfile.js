...
const reloadEntries = () => {
  let entries = localStorage.getItem("userchoice");
...
let Entries = reloadEntries();

const displayEntries = () => {
  const entries = reloadEntries();
  ...
  let tablereq = document.getElementById("tablereq");

  // <th class="th">Name</th> inside oneMore head for name
  tablereq.innerHTML = `<table class="table" border="2">
  <tr>
    <th class="th">Name</th>
...
dateELE.addEventListener("change", () => {
  ...
  if (age < 18 || age > 55) {
    dateELE.setCustomValidity("Your age is not in between 18 and 55");
